 ...
 * @author 吴聪帅
 * @Description
 * @Date : 上午12:01 2019/3/11 Modifyby:
 **/
public class HttpXmlRequestDecoder extends AbstractHttpXmlDecoder {
    ...
    protected void decode(ChannelHandlerContext arg0, Object o, List arg2) throws Exception {
        if (o instanceof  FullHttpRequest) {
            ...
            arg2.add(request);
        } else if (o instanceof DefaultFullHttpResponse) {
            DefaultFullHttpResponse defaultFullHttpResponse = (DefaultFullHttpResponse)o;
            if (!defaultFullHttpResponse.getDecoderResult().isSuccess()) {
                //如果HTTP消息本身解码失败，则构造处理结果异常的HTTP应答消息返回给客户端。
                sendError(arg0, HttpResponseStatus.BAD_REQUEST);
                return;
            }
            //通过HttpXmlRequest和反序列化后的Order对象构造HttpXmlRequest实例，最后将它添加到解码结果List列表中。
            HttpXmlRequest request = new HttpXmlRequest(null, decode0(arg0,defaultFullHttpResponse.content()));
            arg2.add(request);
